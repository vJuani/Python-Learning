...
class MarketingGenerationsTests(unittest.TestCase):
    ...
    def test_http_chat_creates_conversation_and_iterates(self):
        ...
        self.assertIn("Preparé 3 propuestas para Libertador 1000.", body)
        self.assertIn("Copy de prueba", body)
        self.assertIn("mkt-result", body)
        self.assertEqual(body.count("Elegir este diseño"), 3)
    ...
    def test_http_chat_italia_visual_and_hacelo_imagen(self):
        ...
        self.assertIn("Preparé 3 propuestas para", asked)
        self.assertIn("Copy de prueba", asked)
        self.assertNotIn("¿Qué estilo querés?", asked)
        page = client.get(location)
        body = page.get_data(as_text=True)
        self.assertIn("Preparé 3 propuestas para", body)
        self.assertIn("Copy de prueba", body)
        self.assertIn("mkt-variants", body)
        self.assertIn("/marketing/assets/", body)
        self.assertIn("Ver texto de la publicación", body)
        ...
        self.assertIn("Preparé 3 propuestas para", thread)
        self.assertIn("mkt-variants", thread)
        # Same copy on a new row (no AI call) so the first 3 proposals stay in the thread.
        self.assertEqual(len(list_marketing_generations(self.org)), before + 1)
        ...
        self.assertEqual(second["parent_generation_id"], first_id)
        self.assertEqual(second["generated_copy"], first["generated_copy"])
        self.assertEqual(len((first.get("generated_data") or {}).get("visual_variants") or []), 3)
