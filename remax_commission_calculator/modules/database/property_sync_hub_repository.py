...
def get_external_agent_mapping(organization_id, source, external_agent_id):
    ...
    try:
        row = connection.execute(
            """
            SELECT id, organization_id, source, external_agent_id, agent_id
            FROM external_agent_mappings
            WHERE organization_id = ? AND source = ? AND external_agent_id = ?
            """,
    ...
    try:
        ...
        if existing:
            cursor.execute(
                ...
                WHERE id = ? AND organization_id = ?
                """,
                (agent_id, existing["id"], organization_id),
            )
        else:
            execute_insert(
                cursor,
                ...
                INSERT INTO external_agent_mappings (
                    organization_id, source, external_agent_id, agent_id, created_at
                )
                VALUES (?, ?, ?, ?, ?)
                """,
                (organization_id, source, identity, agent_id, _now_iso()),
            )
        connection.commit()
    finally:
        connection.close()
